...
namespace sep2020.Tests
{
    public abstract class sep2020TestBase : AbpIntegratedTestBase<sep2020TestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected sep2020TestBase()
        {
        ...
        protected void UsingDbContext(Action<sep2020DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<sep2020DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<sep2020DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<sep2020DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<sep2020DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<sep2020DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<sep2020DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<sep2020DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<sep2020DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<sep2020DbContext, Task<T>> func)
        {
            T result;
